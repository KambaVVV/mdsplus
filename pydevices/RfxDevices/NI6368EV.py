...
class NI6368EV(Device):
    ...
    for i in range(16):
        parts.extend([
            ...
            {'path':'.CHANNEL_%d:EVENT_NAME'%(i+1), 'type':'text', 'value':'luca '}, # EVENTI CON SPAZIO ALLA FINE... NB!!!
    ...
    def saveInfo(self):
        ...
        NI6368EV.ni6368EvFds[self.getNid()] = self.ev_fd
    ...
    def restoreInfo(self):
        ...
        try:
            self.ev_fd = NI6368EV.ni6368EvFds[self.getNid()]
            ...
            try:
                ...
                self.ev_fd = os.open(fileName, os.O_RDWR) # if not present, try to open
            except:
    ...
    class AsynchStore(Thread):
        ...
        def configure(self, device, ev_fd, chanMap, hwChanMap, treePtr, stopAcq):
            self.device = device
            self.ev_fd = ev_fd
        ...
        def run(self):
            ...
            for chan in range(numChans):
                ...
                getattr(self.device, 'channel_%d_calib_param'%(self.chanMap[chan]+1)).putData(Float32(coeff))
                print('SCRITTO')
            ...
            saveList = c_void_p(0)
            NI6368EV.niInterfaceLib.startSaveEV(byref(saveList)) 
            ...
            while not self.stopReq:
                try:
                    ...
                    self.ev_fd, c_int(numChans), chanFd_c, isBurst_c, f1Divs_c, f2Divs_c,
            ...
            status = NI6368EV.niLib.xseries_stop_ai(c_int(self.ev_fd))
...
####################################################################################
    ...
    def init(self):
        ...
        # getting card info
        status = NI6368EV.niInterfaceLib._xseries_get_device_info(c_int(dev_fd), byref(device_info))
        if status:
        ...
        NI6368EV.niLib.xseries_stop_ai(c_int(self.ev_fd))

        # reset analog input segment
        status = NI6368EV.niLib.xseries_reset_ai(c_int(self.ev_fd))
        if status:
            errno = NI6368EV.niInterfaceLib.getErrno()
        ...
        if (clockMode == 'INTERNAL'):
            ...
            if(status == 0):
                ...
                status = NI6368EV.niLib.xseries_set_ai_convert_clock(aiConf, self.XSERIES_AI_SAMPLE_CONVERT_CLOCK_INTERNALTIMING, self.XSERIES_AI_POLARITY_ACTIVE_HIGH_OR_RISING_EDGE)
            ...
            if(status == 0):
                ...
                status = NI6368EV.niLib.xseries_set_ai_scan_interval_counter(aiConf, self.XSERIES_SCAN_INTERVAL_COUNTER_TB3, self.XSERIES_SCAN_INTERVAL_COUNTER_POLARITY_RISING_EDGE, c_int(100),  c_int(2))
        ...
        try:
            status = NI6368EV.niInterfaceLib.xseries_set_ai_conf_ptr(c_int(self.ev_fd), aiConf)
            if(status != 0):
                errno = NI6368EV.niInterfaceLib.getErrno()
...
####################################################################################
    def start_store(self):
        self.debugPrint('================= PXI 6368 EVstart store ===============')
        ...
        self.worker.configure(self.copy(), self.ev_fd, chanMap, self.diffChanMap, treePtr, stopAcq)
